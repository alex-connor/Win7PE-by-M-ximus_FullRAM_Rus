...
private const L_HelpConfigAddress_006_0_Message = "Note: All listening is subject to the IPv4Fitler and IPv6Filter under    "
...
'''''''''''''''''''''
' HELP - QUICKCONFIG
private const X_HelpQuickConfig_001_0_Message = "winrm quickconfig [-quiet] [-transport:VALUE]"
private const X_HelpQuickConfig_002_0_Message = ""
private const X_HelpQuickConfig_003_0_Message = "Performs configuration actions to enable this machine for remote management."
private const X_HelpQuickConfig_004_0_Message = "Includes:"
private const X_HelpQuickConfig_005_0_Message = "  1. Start the WinRM service"
private const X_HelpQuickConfig_006_0_Message = "  2. Set the WinRM service type to auto start"
private const X_HelpQuickConfig_007_0_Message = "  3. Create a listener to accept request on any IP address"
private const X_HelpQuickConfig_008_0_Message = "  4. Enable firewall exception for WS-Management traffic (for http only)"
...
private const X_HelpQuickConfig_011_0_Message = "If present, quickconfig will not prompt for confirmation."
private const X_HelpQuickConfig_012_0_Message = "-transport:VALUE"
private const X_HelpQuickConfig_013_0_Message = "----------------"
private const X_HelpQuickConfig_014_0_Message = "Perform quickconfig for specific transport."
private const X_HelpQuickConfig_015_0_Message = "Possible options are http and https.  Defaults to http."
...
'wscript.echo analysisOutputXml
    ...
    On Error Resume Next
    Set analysisOutput = CreateObject("MSXML2.DOMDocument.3.0")
    If Err.number <> 0 Then
        stdErr.WriteLine GetResource("L_ERR_Message") & GetResource("L_MSXML3MISSING_Message")
        Exit Function
    End If
    ...
    If (enabled = "true") Then
        If (serviceOnly = true) Then
            stdOut.WriteLine GetResource("L_QuickConfigNoServiceChangesNeeded_Message")
        Else
...
'wscript.echo updateInputXml
    ...
    If (Not cmdlineOptions.ArgumentExists(NPARA_QUIET)) Then
...
'wscript.echo updateOutputXml
    ...
    On Error Resume Next
    Set updateOutput = CreateObject("MSXML2.DOMDocument.3.0")
    If Err.number <> 0 Then
        stdErr.WriteLine GetResource("L_ERR_Message") & GetResource("L_MSXML3MISSING_Message")
        Exit Function
    End If
...
private function ReFormat(rawStr,formattedStr,formatOption)
    ...
    if LCase(formatOption) = VAL_FORMAT_XML then
        ...
        if Err.number <> 0 then
            stdErr.WriteLine GetResource("L_MSXML3MISSING_Message")
            on error goto 0
            ReFormat = false
            exit function
        end if
        ...
        if LCase(formatOption) = VAL_FORMAT_TEXT then
            If Not xslFile.load(VAL_FORMAT_TEXT_XSLT) Then
                stdErr.WriteLine GetResource("L_XSLERROR_Message") & VAL_FORMAT_TEXT_XSLT
        ...
        elseif LCase(formatOption) = VAL_FORMAT_PRETTY then    
            If Not xslFile.load(VAL_FORMAT_PRETTY_XSLT) Then
                stdErr.WriteLine GetResource("L_XSLERROR_Message") & VAL_FORMAT_PRETTY_XSLT
                if xslFile.parseError.errorCode < 0 then
                    stdErr.WriteLine xslFile.parseError.reason
                end if
                on error goto 0
...
GetResource("X_HelpQuickConfig_003_0_Message") & NL & _
GetResource("X_HelpQuickConfig_004_0_Message") & NL & _
GetResource("X_HelpQuickConfig_005_0_Message") & NL & _
GetResource("X_HelpQuickConfig_006_0_Message") & NL & _
GetResource("X_HelpQuickConfig_007_0_Message") & NL & _
GetResource("X_HelpQuickConfig_008_0_Message") & NL & _
...
GetResource("X_HelpQuickConfig_011_0_Message") & NL & _
GetResource("L_Help_Blank_0_Message") & NL & _
GetResource("X_HelpQuickConfig_012_0_Message") & NL & _
GetResource("X_HelpQuickConfig_013_0_Message") & NL & _
GetResource("X_HelpQuickConfig_014_0_Message") & NL & _
GetResource("X_HelpQuickConfig_015_0_Message") & NL & _
GetResource("L_Help_Blank_0_Message") & NL & _
GetResource("L_Help_SeeAlso_Title_Message") & NL & _
