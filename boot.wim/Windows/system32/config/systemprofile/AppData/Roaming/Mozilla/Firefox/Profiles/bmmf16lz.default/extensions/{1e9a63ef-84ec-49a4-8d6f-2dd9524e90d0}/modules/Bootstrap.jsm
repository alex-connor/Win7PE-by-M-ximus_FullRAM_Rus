...
let publicURL = ioService.newURI("chrome://adblocklite-modules/content/Public.jsm", null, null);
...
/**
 * Allows starting up and shutting down Adblock Lite functions.
 * @class
 */
var Bootstrap =
{
	...
	startup: function()
	{
		...
		registrar.registerFactory(cidPublic, "Adblock Lite public module URL", contractIDPublic, factoryPublic);
		registrar.registerFactory(cidPrivate, "Adblock Lite private module URL", contractIDPrivate, factoryPrivate);
		...
		let enumerator = categoryManager.enumerateCategory("adblock-lite-module-location");
	...
	loadModule: function(/**String*/ url)
	{
		...
		{
			Cu.reportError("Adblock Lite: Failed to load module " + url + ": " + e);
			return;
		}
		...
		for each (let obj in module)
		{
			if ("startup" in obj)
			{
				...
				{
					Cu.reportError("Adblock Lite: Calling method startup() for module " + url + " failed: " + e);
				}
				return;
			}
		}

		Cu.reportError("Adblock Lite: No exported object with startup() method found for module " + url);
	},
	...
	shutdownModule: function(/**String*/ url)
	{
		...
		if ("shutdown" in obj)
		{
			...
			{
				Cu.reportError("Adblock Lite: Calling method shutdown() for module " + url + " failed: " + e);
			}
			return;
		}
	}
...
var BootstrapPrivate =
{
	...
	{
		if (data != "adblock-lite-module-location")
			return;
