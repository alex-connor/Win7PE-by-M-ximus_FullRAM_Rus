...
var NoDefault = false;
var gColorObj;
var esbColorPicker_gBundle = Components.classes["@mozilla.org/intl/stringbundle;1"].getService(Components.interfaces.nsIStringBundleService);
var esbColorPickerStrings = esbColorPicker_gBundle.createBundle("chrome://extendedstatusbar/locale/EdColorPicker.properties");
...
function changeHex()
{
   ...
   // XXX Check to see if they are hex digits
    ...
    var myregexp1 = new RegExp("^#[0-9ABCDEF]{6}", "i");
    if (hex.match(myregexp1) == null)

   //if (hex.length < 6)
   {
     //alert("Color is not made of a hash ('#') followed by six hex digits");
     alert(esbColorPickerStrings.GetStringFromName("esbColorPicker.colornohash"));
     return false;
   }
...
function ValidateData()
{
  ...
  if (!gColor)
  {
    ...
    alert(esbColorPickerStrings.GetStringFromName("esbColorPicker.badcolorinput"));
