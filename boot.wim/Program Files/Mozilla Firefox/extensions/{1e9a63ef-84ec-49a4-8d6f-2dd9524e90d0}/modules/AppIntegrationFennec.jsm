...
try
{
	Utils.parentMessageManager.addMessageListener("AdblockLite:Policy:shouldLoad", function(message)
	{
	...
	Utils.parentMessageManager.addMessageListener("AdblockLite:ElemHide:styleURL", function(message)
	{
		return ElemHide.styleURL;
	});

	Utils.parentMessageManager.addMessageListener("AdblockLite:ElemHide:checkHit", function(message)
	{
		try
		{
			let data = message.json;
	...
	FilterStorage.addObserver(function(action)
	{
		if (action == "elemhideupdate")
			Utils.parentMessageManager.sendAsyncMessage("AdblockLite:ElemHide:updateStyleURL", ElemHide.styleURL);
		else if (/^(filters|subscriptions) (add|remove|enable|disable|update)$/.test(action))
			Utils.parentMessageManager.sendAsyncMessage("AdblockLite:Matcher:clearCache");
	});
...
var AppIntegrationFennec =
{
	initWindow: function(wrapper)
	{
		if ("messageManager" in wrapper.window)
		{
			...
			for (let i = 0; i < browsers.length; i++)
				browsers[i].messageManager.loadFrameScript("chrome://adblocklite/content/fennecContent.js", true);
			wrapper.E("tabs").addEventListener("TabOpen", function(event)
			{
				...
				tab.browser.messageManager.loadFrameScript("chrome://adblocklite/content/fennecContent.js", true);
			}, false);

			// Get notified about abp: link clicks for this window
			wrapper.window.messageManager.addMessageListener("AdblockLite:LinkClick", function(message)
			{
	...
	{
		wrapper.window.importDialog(null, "chrome://adblocklite/content/ui/fennecSubscription.xul");
...
{
	if (event.originalTarget.getAttribute("addonID") != "{1e9a63ef-84ec-49a4-8d6f-2dd9524e90d0}")
		return;

	wrapper.E("adblocklite-subscription-list").addEventListener("command", function(event)
	{
	...
	let syncSetting = wrapper.E("adblocklite-sync");
	let syncEngine = Sync.getEngine();
	syncSetting.hidden = !syncEngine;
	syncSetting.value = syncEngine.enabled;
	wrapper.E("adblocklite-sync").addEventListener("command", AppIntegration.toggleSync, false);
...
function updateSubscriptionList(wrapper)
{
	...
	xhr.open("GET", "chrome://adblocklite/content/ui/subscriptions.xml", false);
	xhr.send(null);
	if (!xhr.responseXML)
		return;

	let menu = wrapper.E("adblocklite-subscription-list");
	menu.removeAllItems();
