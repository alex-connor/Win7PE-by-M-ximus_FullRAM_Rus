...
var FilterStorage =
{
	...
	get sourceFile()
	{
		...
		if (!file)
			Cu.reportError("Adblock Lite: Failed to resolve filter file location from extensions.adblocklite.patternsfile preference");
	...
	loadFromDisk: function(silent)
	{
		...
		if (!realSourceFile || !realSourceFile.exists())
		{
			...
			let patternsURL = Utils.ioService.newURI("chrome://adblocklite-defaults/content/patterns.ini", null, null);
		...
		while (true)
		{
			...
			{
				Cu.reportError("Adblock Lite: Failed to read filters from file " + realSourceFile.path);
				Cu.reportError(e);
			}
	...
	saveToDisk: function()
	{
		...
		let buf = ["# Adblock Lite preferences", "version=" + formatVersion];
 ...
 * groups (Adblock Lite 0.7.x data) these filters are returned - they need to be added
 * separately.
 */
